...
1. 原 `test.py` 即使子检查失败也不会返回非零退出码，已修复。
2. 原 `build.py --verify` 只检查验证脚本是否存在，未真实执行，已改为运行完整测试。
3. 新增 `audit_html.py`，检查JS语法、重复函数、重复DOM ID和活动代码失效DOM引用。
4. 2026-06-20 浏览器回归发现主页面因缺失闭合标签被嵌套进隐藏面板；静态关键词测试全部误判为通过。已修复，并将HTML层级加入自动审计。
5. 无UI入口的旧监控、开关箱和导入函数已删除，统一计算链路保持唯一。
6. 未安装watchdog时 `serve.py` 曾因未定义基类而启动失败，已增加安全降级。
7. 线上部署必须仅包含 `index.html`，禁止把Excel模板、MCP本机路径和开发记录作为公开制品上传。
