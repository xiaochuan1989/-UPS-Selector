# UPS 智能选型助手——产品与技术设计

> 版本：v1.8.34
> 最后更新：2026-09-10
> 文档职责：说明产品能力、系统结构、关键数据流、业务公式和技术边界。

## 1. 产品定位

UPS 智能选型助手是面向售前、销售和技术人员的单文件 Web 工具。它把客户技术要求、产品数据、UPS 容量校核、电池配置和项目汇总整合到一个页面中。

正式交付物只有 `index.html`。用户可直接双击使用；涉及 CDN 文档解析、浏览器安全限制或稳定调试时，建议通过本地 HTTP 服务打开。

### 1.1 核心目标

- 从客户需求中提取关键 UPS 参数并调用 AI 推荐型号。
- 用本地产品数据库约束 AI 输出，降低型号幻觉。
- 完成数据中心容量与冗余架构校核。
- 完成电池正向选型、后备时间反算、开关箱和监控配置。
- 生成项目配置汇总、Excel 清单和客户可读技术说明。

### 1.2 非目标

- 本工具不是最终电气设计软件，不能替代厂家曲线、短路计算、选择性保护和施工图设计。
- 页面内访问码和价格查看码仅是客户端使用门槛，不构成真正的安全控制。
- AI 输出必须经过产品数据库和工程人员复核，不能直接作为采购承诺。

## 2. 当前能力

| 能力 | 说明 |
|---|---|
| AI 智能选型 | 支持 OpenAI 兼容接口和 Anthropic 接口，多轮追问，合法型号过滤 |
| 产品数据库 | 内置 105 款 UPS，支持 Excel 导入、IndexedDB 持久化、单型号全景卡片、型号收藏及收藏/关键词组合筛选；32 条数据库行带 iTeaQ-2026-04-V1.0 目录中实际存在的机柜/模块编码和单价，同型号多版本并列展示，组合式 UPS 的编码分行标明机柜与功率模块 |
| 数据中心校核 | 支持 N、N+1、2N、分布式冗余容量及风险提示 |
| UPS 与电池配置 | 统一计算 UPS、功率模块、电池、开关箱和监控；型号可留空并按手动容量或负载计算，加入项目时仍须填写完整型号；同一项目可保存多套不同 UPS 配置，项目清单自动合并相同物料，并支持自定义行、排序、删除、非标描述、编码/单价和快速报价单导出 |
| 电池后备时间反算 | 根据恒功率曲线估算现有电池配置的后备时间 |
| 独立电池计算 | 铅酸方法一和锂电池容量计算 |
| 文档输入 | TXT、MD、DOCX、PDF；图片通过支持视觉的 AI 模型识别 |
| 结果输出 | 推荐方案 HTML、项目汇总 Excel、技术说明 HTML、历史记录 Excel |
| 历史记录 | localStorage 保存最多 50 条选型记录，可搜索、恢复和导出 |

## 3. 数据规模

| 数据集 | 数量 | 位置 |
|---|---:|---|
| UPS 产品 | 105 | `index.html` 中 `PRODUCTS` |
| 太行 UR | 16 | `PRODUCTS` |
| 昆仑 UE | 31 | `PRODUCTS` |
| 祁连 UM | 27 | `PRODUCTS` |
| 泰山 UT | 31 | `PRODUCTS` |
| 电池恒功率型号 | 91 | `index.html` 中 `BATTERY_POWER_DATA` |
| JYC-HR12V 高功率 | 13 | `BATTERY_POWER_DATA`（category `jyc`） |
| JYC-GFM-2V 常规 | 10 | `BATTERY_POWER_DATA`（category `jyc2v`） |
| JYC-HR-2V 高倍率 | 6 | `BATTERY_POWER_DATA`（category `jyc2vhr`） |
| 电池监控产品 | 15 | `index.html` 中 `MONITOR_PRODUCTS` |

电池类别：

- `normal`：SP12V
- `highpower`：SPG12V 高功率
- `2v`：GFMD-2V
- `dianneng`：电能 GBT12-20 小时率
- `dianneng10h`：电能 GBT12-10 小时率
- `jyc`：JYC-HR12V 高倍率（HR 系列 13 款，终止电压 1.60/1.67/1.70/1.75/1.80V）
- `jyc2v`：JYC-GFM-2V 常规（GFM 系列 10 款，终止电压 1.65/1.70/1.75/1.80V）
- `jyc2vhr`：JYC-HR-2V 高倍率（HR 系列 6 款，终止电压 1.65/1.70/1.75/1.80V）
- `custom`：客户指定型号，不进行内置型号推荐

## 4. 系统结构

```mermaid
flowchart TD
    UI["index.html UI"] --> AI["AI 选型与多轮对话"]
    UI --> DC["数据中心方案校核"]
    UI --> BAT["电池正算与反算"]
    UI --> DB["产品数据库查看与 Excel 导入"]
    AI --> PRODUCTS["PRODUCTS 105款 UPS"]
    BAT --> POWER["BATTERY_POWER_DATA 91款"]
    BAT --> MONITOR["MONITOR_PRODUCTS 15款"]
    DB --> IDB["IndexedDB 导入数据"]
    DB --> FAVORITES["localStorage 产品收藏"]
    AI --> HISTORY["localStorage 选型历史"]
    DC --> SUMMARY["projectSummary"]
    BAT --> SUMMARY
    AI --> SUMMARY
    SUMMARY --> EXPORT["Excel / HTML 输出"]
```

### 4.1 单文件架构

单文件是明确的产品约束，而不是尚未完成的拆分：

- 便于内部人员复制和直接打开。
- 产品和电池数据可内嵌，基础选型不依赖后端。
- GitHub Pages 和 Netlify 只需发布一个文件。

代价是文件较大，因此通过以下方式控制复杂度：

- 每类业务保持单一实现，不保留失去 UI 入口的旧函数。
- 关键公式提炼为纯函数并独立测试。
- 自动检查 HTML 层级、DOM ID、函数重复和 JavaScript 语法。
- 文档不记录易漂移的绝对行号，以函数名和数据名定位。

## 5. 主要模块

### 5.1 启动与配置

| 标识 | 职责 |
|---|---|
| `APP_VERSION` | 页面与文档版本的唯一代码来源 |
| `checkAuth` | 客户端访问门槛 |
| `showMainTab` | 顶部标签页导航，切换"智能选型/UPS与电池配置/后备反算/电池计算/数据中心/产品数据库"的单视图 |
| `saveConfig` / `getBaseUrl` | 保存和读取 AI 接口配置 |
| `getCustomPrompt` | 返回自定义或默认系统提示词 |
| `DEBUG_MODE` / `debugLog` | 控制开发日志，生产默认关闭 |

### 5.2 AI 选型

| 函数 | 职责 |
|---|---|
| `buildProductSummary` | 构建发送给 AI 的压缩产品摘要 |
| `callAI` | 统一调用 OpenAI 兼容接口或 Anthropic |
| `analyze` | 首次选型主流程 |
| `validateModels` | 删除不在合法型号列表中的 AI 推荐 |
| `renderResult` | 渲染推荐和备选产品 |
| `sendFollowup` | 多轮追问 |

AI 结果约定：

```json
{
  "analysis": "需求解析、容量判断、匹配逻辑和风险提示",
  "recommended": [
    { "model": "合法产品型号", "reason": "推荐原因" }
  ],
  "also_consider": [
    { "model": "合法产品型号", "reason": "备选原因" }
  ]
}
```

### 5.3 产品数据

| 函数 | 职责 |
|---|---|
| `importExcel` | 解析产品 Excel，并合并基础表与泰尔规格表 |
| `openDatabase` | 打开 IndexedDB |
| `saveProductsToDB` | 持久化导入数据 |
| `loadProductsFromDB` | 页面启动时恢复导入数据 |
| `restoreBuiltinData` | 清除导入数据并恢复内置数据库 |
| `buildDataTable` | 构建双栏产品数据库视图，并为每行绑定型号全景卡片入口 |
| `openDatabaseProductDetail` | 打开单型号详情弹窗，汇总产品信息、关键指标和技术规格（泰尔参数） |
| `toggleDatabaseFavorite` | 在双表或详情卡片中收藏/取消收藏型号，并同步界面和本地存储 |
| `toggleDatabaseFavoritesFilter` | 切换“只看收藏”，并与关键词搜索组合过滤产品 |

### 5.4 数据中心校核

| 函数 | 职责 |
|---|---|
| `updateDcArchitectureFields` | 根据架构切换台数字段 |
| `importDcFromCurrentSelection` | 从统一配置或 AI 首选型号导入 |
| `calcDataCenterCheck` | 计算装机容量、可用容量、负载率和风险 |

### 5.5 电池与项目配置

| 函数 | 职责 |
|---|---|
| `calcAllBatteryConfig` | 当前唯一正式的统一电池配置流程 |
| `renderUpsModelSearchResults` | 按型号关键字实时过滤产品库并渲染可滚动候选列表 |
| `handleUpsModelSearchKeydown` | 处理型号搜索的方向键、回车确认和 Esc 关闭 |
| `saveCurrentUpsConfigurationAndStartNext` | 计算并保存当前 UPS 配置，然后保留公共电池参数继续新增下一套 |
| `collectSavedConfigurationRows` | 汇总多套 UPS 配置，并按物料类别、型号及单位合并相同物料数量；电池保留各套数量计算明细 |
| `calculateBatteryElectricals` | 纯函数：单体功率、总电流、分组电流、传感器电流 |
| `getNextBreakerSize` | 纯函数：选择大一档标准断路器 |
| `selectCurrentSensor` | 纯函数：选择电流传感器量程 |
| `getVoltLevel` | 纯函数：按最大充电电压选择 DC 电压等级 |
| `calcBatteryRuntime` | 按恒功率曲线反算后备时间 |
| `renderProjectSummary` | 渲染统一项目清单 |
| `exportProjectSummary` | 导出项目配置 Excel |
| `exportTechnicalBrief` | 导出客户可读技术说明 HTML |

多套 UPS 的界面动作必须区分：

1. 顶部“开始新配置”只负责清空当前草稿、滚动到参数区并聚焦型号输入，不直接生成配置卡片。
2. 底部“加入项目并新增下一套”先验证 UPS 型号，再计算、保存当前配置并生成卡片。
3. 标准型号必须从产品列表选择；非标型号必须填写名称。两种模式都禁止以空型号加入项目。
4. 已保存配置可载入修改；编辑期间从聚合行中暂时移除，保存后重新参与汇总，避免重复计算。

## 6. 核心数据流

### 6.1 AI 选型

1. 用户输入需求或上传文档。
2. `analyze` 组合需求、系统提示词、产品摘要和合法型号列表。
3. `callAI` 调用模型。
4. `validateModels` 过滤非法型号。
5. `renderResult` 展示结果。
6. `saveSelectionHistory` 保存快照。

### 6.2 统一电池配置

1. 读取 UPS、负载、电池和监控参数。
2. 输入校验。
3. `calculateBatteryElectricals` 计算关键电气量。
4. 从 `BATTERY_POWER_DATA` 中选择满足恒功率要求的型号。
5. 计算开关规格、电压等级和传感器量程。
6. 生成监控配置。
7. 写入 `window.projectSummary`。
8. 用户选择加入项目时，在持久化边界再次校验 UPS 型号，保存到 `window.savedUpsConfigurations`。
9. 按物料身份合并各套配置数量，保留配置来源和电池分项计算说明。
10. 渲染和导出项目汇总。

### 6.3 后备时间反算

1. 计算每台 UPS、每组电池、每只电池承担的功率。
2. 获取指定截止电压下的恒功率曲线。
3. 在相邻功率点之间线性估算时间。
4. 对目标时间给出满足、临界或不满足提示。
5. 客户指定电池没有内置曲线时只输出所需功率，不伪造时间。

## 7. 关键业务公式

### 7.1 电池单体功率

```text
负载功率 loadW =
  已填写负载功率 × 1000
  否则 UPS容量 × 功率因数 × 1000

单体所需功率 =
  loadW ÷ (每组电池节数 × 单格数 × 逆变效率 × 每台UPS电池组数)

单格数：
  12V电池 = 6
  2V电池  = 1
```

### 7.2 电池开关箱电流

开关箱必须按 UPS 额定功率计算，不使用可选负载功率：

```text
switchW = UPS容量 × 功率因数 × 1000

最大放电电流 =
  switchW ÷ (每组电池节数 × 截止电压 × 单格数 × 逆变效率)

分开关电流 =
  最大放电电流 ÷ (电池组数 - 1)，仅组数 ≥ 2 时有效
```

### 7.3 传感器量程

```text
电池组数 = 1：
  传感器电流 = 最大放电电流

电池组数 ≥ 2：
  传感器电流 = 分开关电流
```

| 计算电流 | 产品编码 | 量程 |
|---:|---|---|
| ≤100A | 88091139 | 0~100A |
| ≤200A | 88091140 | 0~200A |
| ≤400A | 88091141 | 0~400A |
| ≤600A | 88091142 | 0~600A |
| ≤800A | 88091143 | 0~800A |
| >800A | 88091144 | 0~1000A |

### 7.4 断路器规格

从标准规格数组中选择严格大于计算电流的第一档；超过最大档位时返回 1600A，并要求工程复核。

### 7.5 电压等级

```text
最大充电电压 = 2.35V × 单格数 × 电池节数

≤250V  → 250VDC
≤500V  → 500VDC
>500V  → 750VDC
```

### 7.6 电池架铜排或电缆

```text
单组电池：采用主开关规格
多组电池：采用分开关规格
```

### 7.7 数据中心架构

- N：全部 UPS 共同承担负载，无冗余故障后容量概念。
- N+1：输入 N 台工作/并机 UPS，总台数为 N+1，故障后按 N 台容量校核。
- 2N：A、B 两路分别输入台数，任一路必须独立承担全部负载。
- 分布式冗余：先按近似 N+1 校核，并明确提示结合实际母线拓扑复核。

## 8. 状态与存储

| 存储 | 键/对象 | 内容 |
|---|---|---|
| localStorage | `ups_config` | AI 提供商、模型、API Key、Base URL |
| localStorage | `ups_custom_prompt` | 自定义系统提示词 |
| localStorage | `ups_selection_history` | 最多 50 条选型历史 |
| localStorage | `ups_db_favorites_v1` | 产品数据库已收藏的 UPS 型号 |
| IndexedDB | `ups_data_db` | 用户导入的产品数据 |
| 内存 | `window.projectSummary` | 当前正在编辑的单套 UPS 配置汇总 |
| 内存 | `window.savedUpsConfigurations` | 已加入项目的多套 UPS 配置及其输入、结果和物料快照 |
| 内存 | `lastResult` | 当前 AI 选型结果 |

API Key 保存在浏览器本地存储中，适合受控内部环境，不适合公共终端。

## 9. 外部运行依赖

正式页面通过 CDN 加载：

- `xlsx-js-style`：Excel 读写和样式。
- `mammoth.js`：DOCX 文本提取。
- `pdf.js`：PDF 文本提取。
- `@tabler/icons-webfont`：顶栏与标签页图标（缺失仅图标不显示，不影响功能与布局）。

基础产品查看和内置计算不依赖这些库；Excel、DOCX、PDF 功能在 CDN 不可用时应显示明确错误。

本地开发依赖：

- Python 标准库：HTML 层级和 DOM 结构审计。
- `openpyxl`：Excel 数据检查。
- `watchdog`：可选文件监听。
- Node.js 标准运行时：JavaScript 语法和业务规则测试，无 npm 依赖。

## 10. 安全边界

- 客户端访问码可从源码读取，只能阻挡误操作。
- 价格查看码同样不是加密或权限系统。
- 线上部署制品只包含 `index.html`，不得发布 Excel 模板、MCP 配置和开发记录。
- `.mcp.json` 是本机 MCP 配置，保留在本地但不纳入 Git。
- 不在代码仓库中写入真实 API Key。
- 导出内容必须对用户输入做 HTML 转义。

## 11. 质量要求

每次发布必须满足：

- 所有内联 JavaScript 可被 Node.js 编译。
- HTML 核心面板层级正确，客户需求卡片不在隐藏容器内。
- DOM ID 和函数定义无重复。
- 活动代码不引用不存在的静态 DOM ID。
- 105 款产品 JSON 可解析。
- 产品编码是有类型约束的标识字段：目录回填值只能是 PDF 原始 8 位数字；多个候选编码以中文分号分隔，部件名称、版本差异和价格说明不得混入编码字段。
- 版本号在页面和三份文档中一致。
- 断路器、电池电气量、传感器和电压等级业务测试通过。
- Playwright 至少覆盖登录、五个主功能面板、数据库和设置/历史弹窗。

## 12. 版本摘要

| 版本 | 日期 | 主要变更 |
|---|---|---|
| v1.4 | 2026-03-20 | UPS、电池、开关箱、监控和项目汇总联动 |
| v1.5 | 2026-03-24 | 电池架和开关品牌选项 |
| v1.6 | 2026-04-01 | 历史记录、图片识别、Excel 行高优化 |
| v1.6.8 | 2026-05-23 | APP_VERSION 单一版本来源 |
| v1.6.9 | 2026-05-23 | 数据中心方案校核 |
| v1.6.17 | 2026-05-23 | 客户指定电池 |
| v1.7.0 | 2026-05-25 | 电池后备时间反算 |
| v1.7.2 | 2026-06-02 | 项目技术说明 HTML 导出 |
| v1.7.3 | 2026-06-20 | 修复主页面层级、开发服务器和汇总序号；清理死代码，新增业务测试、浏览器回归和部署门禁 |
| v1.7.4 | 2026-06-22 | 电池开关标准档位补充 2000A、2500A、3200A、4000A，修复大电流场景错误回落至 1600A |
| v1.7.5 | 2026-07-01 | 项目配置汇总清单支持自定义增加行和上下排序，并同步页面、Excel 和技术说明导出 |
| v1.7.6 | 2026-07-03 | 项目配置汇总清单支持标准行手工补充编码和单价，并在重新计算后保留价格数据 |
| v1.7.7 | 2026-07-03 | 新增快速报价单 Excel 导出，复用汇总清单顺序、编码、单价和自定义行，并启用工作表保护 |
| v1.7.8 | 2026-07-03 | 项目配置汇总清单新增删除行操作，标准行按本次隐藏处理并同步所有导出 |
| v1.7.9 | 2026-07-03 | 汇总清单列名和顺序参考供配电配置清单调整，新增非标描述列并同步导出 |
| v1.8.0 | 2026-07-03 | 桌面端全站主容器放宽，提升汇总宽表和多列配置窗口可读性 |
| v1.8.1 | 2026-07-06 | 调整汇总清单非标描述与备注列顺序，保持页面、Excel、报价单和技术说明一致 |
| v1.8.2 | 2026-07-13 | 新增独立电池类别 `jyc`（JYC-HR12V 高倍率 13 款恒功率型号），接入电池类型下拉、推荐链路和数据库视图，恒功率型号总数增至 75 |
| v1.8.3 | 2026-07-13 | 产品数据源升级至常用UPS速查表 V8.0，内置 PRODUCTS 由 179 款重导为 105 款（太行UR16/昆仑UE31/祁连UM27/泰山UT31），同步页面文案、数据库视图和质量检查数据源 |
| v1.8.4 | 2026-07-13 | 界面视觉刷新（第一阶段）：新增科技蓝设计令牌系统、品牌顶栏与标签页导航（单视图切换）、计算结果三卡改为白卡+彩色标题条，项目配置汇总清单全宽自适应免横向滚动，主容器跟随视口留自适应边距，接入 Tabler 图标 |
| v1.8.5 | 2026-07-13 | 主容器改为居中 + 1600px 最大宽度上限（顶栏、标签页、内容三者对齐居中），避免超宽屏内容铺满拉伸 |
| v1.8.6 | 2026-07-14 | 清理被标签页导航取代的 5 个死函数（toggleBatteryCalc/toggleBatteryCalcV1/toggleRuntimeCalc/toggleDataCenterCheck/toggleDataTable），同步文档 |
| v1.8.7 | 2026-07-14 | 界面视觉刷新（第二阶段）：登录页改为品牌 logo 方块 + 副标语，风格与顶栏统一 |
| v1.8.8 | 2026-07-14 | 接入 iTeaQ 艾特网能公司 logo：base64 内嵌为 .iteaq-logo，替换顶栏（白底 chip）与登录页的通用图标 |
| v1.8.9 | 2026-08-31 | 接入 JYC-GFM-2V 常规 10 款与 JYC-HR-2V 高倍率 6 款规格书数据，支持推荐、反算、动态有效电压和 2V 监控模块；对厂家 GFM-200 重复曲线显示复核风险 |
| v1.8.10 | 2026-08-31 | 优化 JYC 2V 高倍率电池型号命名，在功率值前加入 `2V` 标识（如 `HR-2V500W`），便于与其他电压等级区分 |
| v1.8.11 | 2026-08-31 | 电池开关电流新增计算依据下拉选择，可按 UPS 容量×功率因数或负载功率计算；结果和汇总备注同步记录计算依据 |
| v1.8.12 | 2026-09-05 | 扩大产品数据库“产品信息/技术规格”双表可视区域；新增水平拖拽分隔条、键盘微调、双击均分与高度比例记忆；宽表支持按住内容左右拖动和 Shift+滚轮横向浏览 |
| v1.8.13 | 2026-09-05 | 产品数据库两张表新增可调列宽：拖动表头右侧边界即可放大或缩小列，双击恢复默认，并自动记忆各列宽度 |
| v1.8.14 | 2026-09-05 | 产品数据库卡片和双表区域改为自动占满浏览器剩余高度，移除大屏幕下固定最大高度造成的底部空白，页脚随内容贴近窗口底部 |
| v1.8.15 | 2026-09-05 | 修复产品数据库普通鼠标拖动被横向平移逻辑拦截、无法选择文字的问题；普通拖动恢复文字选择，Shift+拖动或中键拖动用于横向查看 |
| v1.8.16 | 2026-09-05 | 产品数据库新增单型号数据全景卡片：单击任一行即可集中查看关键指标、全部产品信息和技术规格（泰尔参数）；保留文字选择、Shift 横移和键盘打开能力 |
| v1.8.17 | 2026-09-05 | UPS 与电池配置新增多套 UPS 项目模式：每套配置独立计算、保存、载入修改和删除；项目汇总自动合并相同型号物料数量，并统一用于 Excel、报价单和技术说明导出 |
| v1.8.18 | 2026-09-05 | 优化多套 UPS 新增流程：顶部按钮明确为“开始新配置”并自动定位、聚焦型号输入；未选择或填写 UPS 型号时禁止加入项目，避免生成空型号配置 |
| v1.8.19 | 2026-09-05 | 标准 UPS 型号选择升级为可搜索组合框：支持任意关键字实时过滤、匹配数量提示、鼠标选择和键盘导航；加入项目时要求选择产品库中的完整型号 |
| v1.8.20 | 2026-09-06 | 产品数据库新增型号收藏：上下双表和型号详情卡片均可收藏/取消收藏，支持“只看收藏”与关键词组合筛选，并通过 localStorage 持久化 |
| v1.8.21 | 2026-09-06 | 严格对照 UPS 速查表的型号与描述、以及 iTeaQ-2026-04-V1.0 目录价：为 12 条无歧义 UPS 回填产品编码和目录价；模块化 UPS 保留机柜与功率模块单价，并对导入/持久化数据执行同一严格匹配 |
| v1.8.22 | 2026-09-06 | 扩展 UPS 目录价覆盖：从 12 条增加至 32 条数据库行；同型号高效型/标准型、不同柜深版本全部并列保留；组合式 UM 可回填目录中实际存在的模块或部分机柜，并明确标注未收录组件 |
| v1.8.23 | 2026-09-07 | 修复 UPS 汇总状态回归：标准模式禁止空型号计算；已有结果切换型号时自动重算；参数修改后提示重新计算；祁连 UM 机柜/功率模块兼容空格与换行分隔，并自动带入可确定的目录编码和价格 |
| v1.8.24 | 2026-09-07 | 恢复多机框版本模块化 UPS 拆分：支持同一数据库记录包含一至多个机框型号并以功率模块结尾，汇总清单按 UPS 型号生成机框和模块两行及对应数量、编码和目录价 |
| v1.8.25 | 2026-09-07 | 优化项目汇总清单：标准 UPS 使用数据库完整描述并支持换行显示；相同功率模块不再因每台模块数描述不同而拆行，数量自动求和并分行保留配置说明；表头支持左右拖动调整列宽、双击恢复并持久化宽度 |
| v1.8.26 | 2026-09-07 | 汇总清单首次按业务顺序自动排列：UPS/机框、功率模块、电池、电池开关箱/柜、电池架、电池监控；监控物料按产品定义顺序排列，旧版自动顺序迁移为新顺序，同时保留上下箭头手动微调 |
| v1.8.27 | 2026-09-07 | 汇总清单序号列新增鼠标拖拽手柄，可直接将任意行拖到目标行前后并显示蓝色落点提示；拖后顺序同步页面及导出，上下箭头继续用于精确微调 |
| v1.8.28 | 2026-09-07 | 汇总清单自定义行支持从 Excel 按起始单元格批量粘贴多行多列，自动补足缺少的自定义行；数量和单价自动转为数值，小计列保持系统计算并跳过，粘贴结果同步全部导出 |
| v1.8.29 | 2026-09-07 | 修复 Excel 粘贴长描述显示不全：汇总表中的自定义行、非标 UPS 和电池描述框在粘贴、输入及重新渲染后按内容自动增高，表格行高同步展开且不再显示内部纵向滚动条 |
| v1.8.30 | 2026-09-08 | 修复多套配置错误合并：电池开关箱/柜、电池架、非标及自定义物料只有型号与配置描述均一致时才汇总数量；不同开关电流或结构规格保持独立行；产品名称列不再重复显示“配置 1/配置 2”，来源仅保留在备注中 |
| v1.8.31 | 2026-09-09 | 修复 UPS 目录产品编码字段：32 条回填数据只显示 PDF 原始 8 位数字，多编码以中文分号分隔；机柜/模块名称继续保留在目录价说明中，汇总清单按同序目录价语义正确拆分机柜与模块编码；新增全量编码格式门禁 |
| v1.8.32 | 2026-09-10 | 放宽“计算当前配置”的型号校验：标准或非标 UPS 型号均可留空，按手动输入的容量或负载完成电池、开关和监控计算；已输入但未完整选择的标准型号仍会提示纠正，加入项目继续要求完整型号 |
| v1.8.33 | 2026-09-10 | 优化产品数据库组合式 UPS 的编码展示：依据同序目录价语义，将多编码分行标注为机柜或功率模块；例如 `01021107` 明确显示为 `50kVA功率模块`，详情卡片同步展示，底层编码继续保持纯 8 位数字 |
| v1.8.34 | 2026-09-10 | 新增电池监控主机/显示屏数量下拉选项：保留原有按总电池组数每6组1套的默认算法，并增加按UPS台数每台独立1套；电流模块、传感器、单体模块、测试线和组电压模块仍按原规则计算 |

更细的历史变更以 Git 提交记录为准。
