...
- 多套 UPS 项目：同一项目可保存多套不同 UPS 与电池配置；顶部“开始新配置”用于定位并清空填写区，底部“加入项目并新增下一套”用于保存当前配置；支持载入修改、删除和连续新增，项目汇总会自动合并相同型号物料数量并保留配置来源；标准模式必须选择完整 UPS 型号后才能计算，祁连 UM 机柜与功率模块会分行进入清单并自动带入可确定的目录编码/价格
...
- 汇总导出：`项目配置汇总清单` 区域支持自定义增加行、从 Excel 多行多列批量粘贴并自动补行，描述框和表格行高随内容自动展开；支持鼠标拖拽或上下箭头调整顺序、删除当前行、非标描述、手工补充编码/单价，并导出 Excel 清单、快速报价单和客户可读 HTML 技术说明
