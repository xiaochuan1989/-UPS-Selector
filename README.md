...
- 多套 UPS 项目：同一项目可保存多套不同 UPS 与电池配置；顶部“开始新配置”用于定位并清空填写区，底部“加入项目并新增下一套”用于保存当前配置；支持载入修改、删除和连续新增；项目汇总仅合并型号和配置规格均一致的物料，电池开关柜、电池架、非标/自定义物料描述不同时保持独立行，配置来源只在备注中显示；标准模式必须选择完整 UPS 型号后才能计算，祁连 UM 机柜与功率模块会分行进入清单并自动带入可确定的目录编码/价格
