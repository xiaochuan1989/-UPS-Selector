...
- 顶部 `数据中心方案校核` 是独立功能面板，用于工程架构校核，不替代原 AI 选型和电池配置计算。
...
## v1.7.2 更新记录（2026-06-02）

### 项目技术说明导出
- `项目配置汇总清单` 区域新增 `导出技术说明` 按钮。
- 导出 HTML 文件，整合数据中心方案校核、电池后备时间反算、UPS与电池配置计算、项目配置汇总清单和AI选型需求分析。
- 导出文件可直接浏览器打开、打印或另存为 PDF，便于售前技术澄清和内部方案复核。

---

## v1.7.1 更新记录（2026-05-25）

### 后备时间字段统一与导入同步
- 将 `UPS与电池配置计算` 的 `放电时间` 和反算页的 `目标后备时间 (分钟)` 统一为 `后备时间 (分钟)`。
- `电池后备时间反算` 导入 `UPS与电池配置计算` 参数时，同步带入后备时间。
- `数据中心方案校核` 从 `UPS与电池配置计算` 导入参数时，同步带入后备时间；校核完成后也会把后备时间同步回下方电池配置。

---

## v1.7.0 更新记录（2026-05-25）

### 第二阶段：电池后备时间反算
- 顶部新增 `电池后备时间反算` 独立入口，避免与 `UPS与电池配置计算` 的正向选型流程混淆。
- 支持输入总负载功率、UPS台数、单台UPS容量、功率因数、每台UPS电池组数、每组电池节数、截止电压和目标后备时间。
- 内置电池型号按恒功率数据表反推预计后备时间，使用相邻恒功率点线性估算，并判断是否满足目标后备时间。
- `自定义/客户指定` 电池保留数量和单体功率计算，但不强行反算时间，提示需厂家提供恒功率放电表复核。

---

## v1.6.17 更新记录（2026-05-23）

### 客户指定电池支持
- `UPS与电池配置计算` 的电池类型新增 `自定义/客户指定`。
- 选择自定义电池时，不再从内置恒功率表强行推荐型号，结果区提示可在项目汇总清单中手动输入客户指定电池型号。
- 开关箱、监控、总数量和项目汇总清单仍继续计算，汇总清单中的电池型号保持空白可编辑。

---

## v1.6.16 更新记录（2026-05-23）

### 电池恒功率工况选项同源化
- `UPS与电池配置计算` 的 `放电时间`、`截止电压` 不再使用旧版写死选项，改为从 `BATTERY_POWER_DATA` 按电池类型动态生成。
- `电池恒功率数据表` 弹窗的截止电压筛选同样从恒功率数据生成，分类切换后自动刷新。
- 推荐计算跳过恒功率为空的点，避免把数据表中显示为 `-` 的工况作为候选电池。

---

## v1.6.15 更新记录（2026-05-23）

### N+1文案优化
- 将 `工作UPS台数 (N)` 改为 `并机UPS数量 (N)`。
- 提示语同步改为 `N+1总数 = 并机UPS数量N + 1台冗余`。
- 容量校核说明同步采用 `并机UPS数量N`，更符合售前工程表达习惯。

---

## v1.6.14 更新记录（2026-05-23）

### 数据中心校核与电池配置联动
- 上方 `数据中心方案校核` 完成计算后，会将 `UPS总台数（自动）` 默认同步到下方 `UPS与电池配置计算` 的 `UPS台数`。
- 打开 `UPS与电池配置计算` 面板时，如果上方已有总台数且下方未手动修改，也会自动带入。
- 下方 `UPS台数` 一旦手动输入，会标记为用户覆盖，后续不会被上方自动覆盖。
- 修正 `导入现有UPS参数` 读取字段：从 `unified-ups-count` 改为实际存在的 `unified-ups-num`。

---

## v1.6.13 更新记录（2026-05-23）

### N+1架构数量口径修正
- `N+1` 不再把输入台数理解为总台数，而是改为 `工作UPS台数 (N)`。
- `UPS总台数（自动）` 在 `N+1` 下自动显示为 `N + 1`。
- `N+1` 容量校核新增计算过程：
  - 工作UPS台数N
  - 冗余UPS 1台
  - UPS总台数
  - 任意1台退出后的剩余容量
- 从其他面板导入UPS总台数时，若当前架构为 `N+1`，会自动换算为 `工作UPS台数N = 总台数 - 1`。

---

## v1.6.12 更新记录（2026-05-23）

### N架构校核文案修正
- `N` 架构没有冗余故障后容量概念，结果卡片改为显示 `可用容量`。
- `N` 架构容量校核新增 `UPS并机台数` 计算过程，例如 `100kVA × PF0.95 × 2台 = 190kW`。
- `N` 架构不再显示 `故障/冗余切换后负载率`，避免把普通并机容量误解为故障后容量。
- `N+1`、`2N`、`分布式冗余` 继续显示各自对应的故障后/单路/冗余后容量。

---

## v1.6.11 更新记录（2026-05-23）

### 数据中心方案校核数量模型优化
- 冗余架构与UPS数量输入联动：
  - `N` / `N+1` / `分布式冗余`：显示 `UPS并机台数`
  - `2N`：隐藏并机台数，改为显式填写 `A路UPS台数` 和 `B路UPS台数`
- `UPS总台数` 改为只读自动汇总，避免用户在2N架构下误把总台数、每路台数混用。
- 从 `UPS与电池配置计算` 导入UPS台数时，若当前为2N架构，会自动拆分到A/B路。
- 新增A/B路数量不一致风险提示，提示复核是否为非对称双母线设计。

---

## v1.6.10 更新记录（2026-05-23）

### 数据中心方案校核修正
- `UPS台数` 统一为 `UPS总台数`，与 `UPS与电池配置计算` 面板口径保持一致。
- `2N` 架构不再把输入台数理解为“每路台数”，而是按总台数拆分为A/B两路；例如总台数2台时，单路可用容量为1台UPS容量。
- `导入现有UPS参数` 优先从 `UPS与电池配置计算` 面板读取已填写的UPS型号、容量、功率因数和台数；没有这些参数时再读取AI首选型号。
- 新增2N总台数奇数风险提示，避免A/B两路不对称。

---

## v1.6.9 更新记录（2026-05-23）

### 数据中心方案校核
- 顶部新增 `数据中心方案校核` 独立入口，避免与原 AI 选型、UPS与电池配置计算混淆。
- 新增场景参数：IT负载、扩容余量、目标运行负载率、冗余架构、供电路径、可靠性等级、后备时间、柴发、输入/输出制式。
- 新增UPS配置参数：UPS型号、单台容量、功率因数、UPS台数、模块容量、模块数量。
- 支持 `N`、`N+1`、`2N`、`分布式冗余` 的容量和故障后可用容量校核。
- 支持从当前AI选型首选型号导入UPS型号、容量、功率因数和模块化参数。
- 自动输出缺失参数和工程风险提示，例如单路负载、Tier等级与架构不匹配、容量余量不足、后备时间/柴发未明确等。
